...
export function SafetySolutions() {
  ...
  return (
    <section ref={containerRef} className="py-24 bg-white relative z-10 px-6 border-t border-black/10">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-0 border border-black/10">

        {/* Left Side - Dashboard Metrics */}
        <div className="lg:w-1/2 flex flex-col">
          <div className="p-8 md:p-12 border-b border-black/10">
            <h2 className="text-4xl md:text-5xl font-bold text-[#1a1a1a] leading-tight tracking-tight">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 flex-grow">
            {features.map((feature, index) => (
              <div
                ...
                className={`p-8 border-b sm:border-b-0 sm:border-r border-black/10 flex flex-col justify-between hover:bg-gray-50 transition-colors ${index % 2 === 0 ? "sm:border-b" : "sm:border-r-0 sm:border-b"
                  } ${index >= 2 ? "sm:border-b-0" : ""}`}
              >
                <div>
                  <div className="flex justify-between items-start mb-6">
                    <feature.icon className="w-6 h-6 text-[#1a1a1a]" strokeWidth={1.5} />
                    <span className="font-mono text-xs text-gray-400">{feature.id}</span>
                  </div>
        ...
        <div className="lg:w-1/2 relative min-h-[500px] border-t lg:border-t-0 lg:border-l border-black/10 overflow-hidden bg-[#1a1a1a]">
          <div
            ref={imageRef}
          ...
          <div className="absolute bottom-0 left-0 w-full p-8 md:p-12 flex justify-between items-end border-t border-white/10 backdrop-blur-sm bg-black/20">
            <div>
              <div className="text-5xl md:text-7xl font-mono font-bold text-[var(--primary)] tracking-tighter">100%</div>
              <div className="font-mono text-xs tracking-widest text-gray-400 uppercase mt-2">Compliance Rating</div>
            </div>
            <div className="text-right hidden sm:block">
              <div className="font-mono text-xs tracking-widest text-white uppercase">International</div>
              <div className="font-mono text-xs tracking-widest text-gray-500 uppercase mt-1">Standards Met</div>
            </div>
          </div>
        </div>
